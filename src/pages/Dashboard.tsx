...
export default function Dashboard() {
  const [washers, setWashers] = useState(getWasherStatus());
  const [summary, setSummary] = useState(getTodaySummary());
  const [activeRentals, setActiveRentals] = useState(
    getRentals().filter((r) => r.status === "active")
  );
  const [recentEntries, setRecentEntries] = useState(getTodayEntries().slice(0, 5));

  useEffect(() => {
    const interval = setInterval(() => {
      setWashers(getWasherStatus());
      setSummary(getTodaySummary());
      setActiveRentals(getRentals().filter((r) => r.status === "active"));
      setRecentEntries(getTodayEntries().slice(0, 5));
    }, 3000);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="card-highlight">
          <CardContent className="pt-5 pb-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Activas en Zona</p>
              <p className="stat-value">{washers.active}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="card-success">
          <CardContent className="pt-5 pb-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="stat-value">{washers.warehouse}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="card-warning">
          <CardContent className="pt-5 pb-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="stat-value">{washers.maintenance}</p>
            </div>
          </CardContent>
        </Card>
      </div>
